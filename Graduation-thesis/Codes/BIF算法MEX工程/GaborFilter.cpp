...
CGaborFilter::CGaborFilter()//原始的函数是针对5个尺度和8个方向的(未修改)
{
...
CGaborFilter::CGaborFilter(int Orientation,int Scale)//我写的可以自定义参数（尺寸和方向的）
{
	m_pOriImageData = NULL;
    m_pReal = NULL;     
	m_pImaginary = NULL;
...
//	m_nGaborWinHeight = 64;
	...
	//默认的Gabor_Kernel是64*64的，如果没有设置参数就取默认值。
	if(m_bParamentsSet == false )
	{
...
BOOL CGaborFilter::MemoryFree()
{
	...
	if(m_pKernal != NULL)
	{
		...
		if(m_ppGaborKernelReal != NULL)
		{
	        Delete2DPointer<float>(m_ppGaborKernelReal,m_nScale*m_nOrientation); //这里出错了
			m_ppGaborKernelReal = NULL;
